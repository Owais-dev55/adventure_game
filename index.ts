...
if (playerName.player === "") {
  console.log(
    chalk.green.italic(
      `************PLAYER01,WELCOME TO THE GALAXY GUARDIANS************`
    )
  );
} else {
  console.log(
    chalk.green.italic(
      `************${capatalizedname},WELCOME TO THE GALAXY GUARDIANS************`
    )
  );
}
...
GAME: while (running) {
  ...
  console.log(
    chalk.bold(
      `####### "Beware! ${Enemy.Enemyname} has arrived to challenge you!`
    )
  );
  ...
  while (maxEnemyhealth > 0 && maxPlayerhealth > 0) {
    ...
    if (operation.action === "Attack") {
      ...
      console.log(
        chalk.redBright(`you hit the ${Enemy.Enemyname} for ${damagegiven}`)
      );
      console.log(
        chalk.greenBright(`Enemy hit you ${Enemy.Enemyname} for ${damagetaken}`)
      );
      ...
      if (guardHealer != 0) {
        ...
        console.log(
          `You have no guard healer left! Fight for the chance to get one`
        );
      }
    } else {
  ...
  if (maxPlayerhealth <= 0) {
    console.log(
      "You have taken too much damage. You limp out the Galaxy Guardian!!!"
    );
  ...
  console.log(
    chalk.yellow(
      `RATE US AT "www.linkedin.com/in/owais-khilji-333owais2386556"`
    )
  );
  console.log(chalk.magenta(`##########################`));
}
